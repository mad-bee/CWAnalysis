...
def overview_plot(session: SessionAnalysis, config: ReportConfig, output: Path) -> Path:
    ...
    for axis, kind in zip(axes, ("dit", "dah")):
        values = [v for c in session.characters for e in c.elements if e.element_type == kind for v in e.values]
        if values:
            axis.hist(values, bins="auto", color=config.colours[kind], alpha=0.8, edgecolor="white")
            median = float(np.median(values))
            axis.axvline(median, color=config.colours["median"], linewidth=1.5, label=f"Median {median:.2f}")
            if config.show_reference_lines and math.isfinite(session.median_dit):
                ideal = session.median_dit if kind == "dit" else session.median_dit * 3
                axis.axvline(ideal, color=config.colours["ideal"], linestyle="--", label=f"Ideal {ideal:.2f}")
            axis.legend(fontsize=7, frameon=False)
        axis.set_title(kind.title())
        axis.set_xlabel(config.units)
